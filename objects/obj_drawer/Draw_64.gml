...
if timer >= delay && !stop_draw {
	...
	if string_char_at(test_str,index) == "^" {
		...
		else if string_char_at(test_str,index+1) == "+" {
	...
	else if string_char_at(test_str,index) == "/" {
		if string_char_at(test_str,index + 1) == "E" {
			stop_draw = true
			stop_snd = true
			skippable = true
...
for (var i = 1; i <= string_length(test); i++) {
	...
	if !gaster {
		...
		for (var j = 1; j <= _border_thick; j++) {
			draw_text_color(_tx - j, _ty, _text, _outline_color, _outline_color, _outline_color, _outline_color, alpha)
			draw_text_color(_tx + j, _ty, _text, _outline_color, _outline_color, _outline_color, _outline_color, alpha)
			draw_text_color(_tx, _ty - j, _text, _outline_color, _outline_color, _outline_color, _outline_color, alpha)
			draw_text_color(_tx, _ty + j, _text, _outline_color, _outline_color, _outline_color, _outline_color, alpha)
		}
